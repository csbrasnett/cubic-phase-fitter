...
def derivative(x, y, z, lamb):
    ...
    stacked_arrs = np.stack((t1, t2, t3, t4, t5, t6))

    # this only works with a single coordinate passed, left in for clarity about what's expected
    ...
    derivatives_array = np.array([stacked_arrs.T[i].sum(axis=1) for i in range(stacked_arrs.T.shape[0])])

    return derivatives_array  # np.array([F_x_val, F_y_val, F_z_val, F_xx_val, F_xy_val, F_yy_val, F_yz_val, F_zz_val, F_xz_val])
